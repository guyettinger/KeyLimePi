...
available immediately; call \`load_skill\` with its name when the task comes round again.`
  },
  {
    ...
    // `NOTES.md` with an indexed `memory/` directory: an index of one line per note
    // saying when that note matters, with bodies read on demand. Removed rather than
    // rewritten, because the correction is three skills under different names and a
    // skill whose body no longer describes its own name is worse than one that is gone.
    //
    // One entry covers every install. `git log --follow` over
    // `docs/skills/working-notes/SKILL.md` shows a single commit, and the text carried
    // no brand name, so it survived both renames unchanged — verified by diffing
    // 4f6b266's copy against the one this session deleted.
    body: `# Working Notes
